...
$isMember = db::fetch("select m.* from members m inner join tours t on m.tour_idx = t.idx where m.user_idx = '$user->idx' and m.status = 1 and t.isCompleted = 0");
...
<main class="myPage-content">
  <div class="tour-list">
    ...
    <p class="tour-msg">
      ...
      <?php elseif (db::fetch("select * from members where user_idx = '$user->idx' and status = 0")): ?> 신청하신 축제 탐방 가입을 운영자가 검토 중입니다.
      <?php elseif ($isManager || $isMember):
        $tour = $isManager ?: db::fetch("select * from tours where idx = '$isMember->tour_idx'");
        $tour_members = db::fetchAll("select m.*, u.name, u.id, u.profile from members m inner join users u where m.status = 1 and m.tour_idx = '$tour->idx'");
